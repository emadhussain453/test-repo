...
import { ExTypes, POMELO_IP_ADDRESSES, PomeloCardBLockStatus, PomeloTransactionOrigin, PomeloWebhookMethods, PomeloWebhookStatus, PomeloWebhookStatusDetails, Status, TrnasactionsTypes, feeTypes, EventTypes, NotificationTypes, NotificationTitles, Lenguages, StableModelsNames, TransactionTypes, CountryCurrencies, Applications, ScoreKeys } from "../../constants/index.js";
...
const transactionsDebit = async (req, res, next) => {
  ...
  try {
    ...
    const isPomeloUser = await PomeloUsers.findOne({ pomeloUserId: user.id }).populate({
      path: "userId",
      select: "isBlocked email balance isVerified language firstName lastName devices notificationCount country",
    });
    if (!isPomeloUser) {
    ...
    if (totalAfterAddingFee > userBalance.balance) {
      ...
      return sendErrorResponse(req, res, PomeloWebhookStatus.REJECTED, PomeloWebhookStatusDetails.INSUFFICIENT_FUNDS, "User has insufficent funds");
    }
    ...
    try {
      const checkIfFirstCashin = await ScoreHistory.findOne({ userId: isPomeloUser.userId._id, code: ScoreKeys.CARD_TRANSACTION });
      if (!checkIfFirstCashin) {
        // update user score
        const scoreData = {
          userId: isPomeloUser.userId._id,
          code: ScoreKeys.CARD_TRANSACTION,
        };
...
export default transactionsDebit;
